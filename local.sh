...
echo "🚀 Starting all projects in separate terminals..."
...
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" || "$OSTYPE" == "cygwin" ]]; then
    ...
    cmd.exe //c start "Frontend" bash -c "cd \"$SCRIPT_DIR/frontend\" && $PKG_MGR local; exec bash"

    echo "🟢 Starting backend..."
    cmd.exe //c start "Backend" bash -c "cd \"$SCRIPT_DIR/backend\" && $PKG_MGR local; exec bash"
    ...
    # macOS
    echo "🟢 Starting frontend..."
    osascript -e "tell app \"Terminal\" to do script \"cd \\\"$SCRIPT_DIR/frontend\\\" && $PKG_MGR local\""

    echo "🟢 Starting backend..."
    osascript -e "tell app \"Terminal\" to do script \"cd \\\"$SCRIPT_DIR/backend\\\" && $PKG_MGR local\""
    ...
    if command -v gnome-terminal >/dev/null 2>&1; then
        echo "🟢 Starting frontend..."
        gnome-terminal -- bash -c "cd \"$SCRIPT_DIR/frontend\" && $PKG_MGR local; exec bash"

        echo "🟢 Starting backend..."
        gnome-terminal -- bash -c "cd \"$SCRIPT_DIR/backend\" && $PKG_MGR local; exec bash"

        echo "🟢 Starting AI server..."
        gnome-terminal -- bash -c "cd \"$SCRIPT_DIR/ai-server\" && poetry run python scripts/start.py local; exec bash"
    elif command -v xterm >/dev/null 2>&1; then
        echo "🟢 Starting frontend..."
        xterm -e bash -c "cd \"$SCRIPT_DIR/frontend\" && $PKG_MGR local; exec bash" &

        echo "🟢 Starting backend..."
        xterm -e bash -c "cd \"$SCRIPT_DIR/backend\" && $PKG_MGR local; exec bash" &
        ...
        echo "   Frontend: cd frontend && $PKG_MGR local"
        echo "   Backend: cd backend && $PKG_MGR local"
...
echo ""
echo "Frontend → Check your frontend port"
